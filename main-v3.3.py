...
class ProtobufProcessor:
    ...
    def normalize_body_landmarks(landmarks, left_side):
        ...
        if landmarks.shape[0] < 10:
            return torch.zeros((1, 10, 3))  # Return zero if not enough landmarks
        ...
        if left_side:
            ...
            L = (
                torch.linalg.vector_norm(landmarks[0] - landmarks[2]) +
                torch.linalg.vector_norm(landmarks[4] - landmarks[2])
            ) / 2.0
        else:
            origin = landmarks[1].clone().detach()  # right shoulder
            L = (
                torch.linalg.vector_norm(landmarks[1] - landmarks[3]) +
                torch.linalg.vector_norm(landmarks[3] - landmarks[5])
            ) / 2.0

        L = L if L > 0 else 1.0  # Prevent division by zero

        # Normalize all 10 landmarks using origin and scale
        normalized = (landmarks - origin) / L

        return normalized.unsqueeze(0)  # Shape: (1, 10, 3)
...
class PoseVideoDataset(Dataset):
    ...
    def _load_protobuf(self, pb_path):
        ...
        pose_tensor = torch.empty([0, 10, 3], dtype=torch.float32)
        ...
        if num_frames < 15:
            padding = torch.zeros([15 - num_frames, 10, 3], dtype=torch.float32)
            pose_tensor = torch.cat((pose_tensor, padding), dim=0)
        elif num_frames > 15:
            pose_tensor = pose_tensor[:15, :, :]

        return pose_tensor  # (15, 10, 3)
...
class PoseVideoCNNRNN(nn.Module):
    ...
    def forward(self, pose_input, deterministic=False):
        ...
        # --- Final linear mapping to 7D
        flat = output.view(output.size(0), -1)       # → (B, 15×10×3)
        out_7d = self.fc_output(flat)                # → (B, 15×7)
        out_7d = out_7d.view(-1, 15, 7)               # → (B, 15, 7)

        return out_7d, mu, logvar
...
class ForwardKinematics:
    def __init__(self, urdf_path):
        ...
        self.selected_joints = [
            ...
            "right_Elbow_2",
            "right_Wrist",
            "right_Finger_1_1"  # Assuming this is the new 7th joint
        ]
    ...
    def _precompute_body_references(self):
        """
        Precompute reference positions and lengths using zero joint angles.
        ...
        with torch.no_grad():
            ...
            # Reference length (used to normalize distances)
    ...
    def batch_forward_kinematics(self, batch_joint_values):
        ...
        normalized = torch.stack([
            (pos - shoulder_pos) / self.L_ref for pos in link_positions
        ], dim=1)  # Shape: [B*T, 10, 3]

        normalized = normalized.view(batch_size, seq_len, 10, 3)
        return normalized
